...
export default function TrainingPathDashboardPage({ params }: TrainingPathDashboardPageProps) {
  ...
  useEffect(() => {
    // Check global registration status from local storage
  ...
  const handleRegistrationSuccess = (userName: string, employeeId: string, department: string) => {
    ...
    localStorage.setItem("tata-steel-training-registered", "true") // Set global registration
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          {!isRegistered ? (
            <Card className="mb-8 p-6 text-center">
              <CardTitle className="text-2xl font-semibold text-primary mb-4">
                Enroll to Access Your Dashboard
              </CardTitle>
                              ...
                              <div className="flex items-center space-x-2">
                                {!isModuleCompleted && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => markPathModuleComplete(module.name)}
                                  >
                                    Mark Complete
                                  </Button>
                                )}
      ...
      <Dialog open={showRegistrationModal} onOpenChange={setShowRegistrationModal}>
        <DialogContent className="sm:max-w-md">
          ...
          <RegistrationForm
            itemTitle={pathName}
            onSuccess={(userName, employeeId, department) =>
              handleRegistrationSuccess(userName, employeeId, department)
            }
