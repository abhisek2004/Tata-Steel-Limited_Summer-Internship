...
export default function CourseDetailPage({ course }: CourseDetailPageProps) {
  ...
  const handleRegistrationSuccess = (userName: string, employeeId: string, department: string) => {
    ...
    // Redirect to the course-specific dashboard with userName as a query parameter
    router.push(`/courses/${String(course.id)}/dashboard?userName=${encodeURIComponent(userName)}`)
  }
  ...
  useEffect(() => {
    // Check localStorage for registration status for this specific course
    const registeredCourses = JSON.parse(localStorage.getItem('registeredCourses') || '{}')
    if (registeredCourses[course.id]) {
      setIsRegistered(true)
      setRegisteredUserName(registeredCourses[course.id].userName)
      router.push(`/courses/${String(course.id)}/dashboard?userName=${encodeURIComponent(registeredCourses[course.id].userName)}`)
    }
  }, [course.id, router])
